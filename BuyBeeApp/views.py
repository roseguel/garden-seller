...
from BuyBeeApp.models import Carrito, DetallePedido, Envio, EstadoEnvio, Pedido, ProductoCarrito, ProductoImagen, ProductoVenta, Usuario, Categoria
...
def producto(r, id_producto):
    ...
    carrito = Carrito.objects.filter(duenio=vendedor.rut).values()
    if len(carrito) < 1:
        carritoObject = Carrito(total=0, cupon="", duenio=vendedor)
        carritoObject.save()
        carrito = carritoObject
    carrito = get_object_or_404(Carrito, duenio=vendedor.rut)
    ...
    if r.method == "POST":
        if len(ProductoCarrito.objects.filter(carrito=carrito.id).values("producto")) < 1:
            productoCarrito = ProductoCarrito(carrito=carrito, producto=producto, cantidad=1)
            productoCarrito.save()
        else:
            productoCarrito = ProductoCarrito.objects.filter(Q(carrito=carrito.id) | Q(producto=producto.id)).values()[0]
...
def publicarProducto(r):
    ...
    return render(r, "BuyBeeApp/subir-producto.html")
